...
import { getEvmWallet, getEvmProvider } from '../wallets/evm.js';
...
// Native token addresses for EVM chains (unused but kept for reference)
...
export const executeInchSwap = async (swap: any, chain: any): Promise<any> => {
  try {
    console.log(
    ...
    const routerAddress = (INCH_ROUTER_ADDRESSES as any)[inchChainId];
    if (!routerAddress) {
    ...
    try {
      ...
      const swapData = swapResponse.data as any;
...
function isNativeToken(tokenAddress: string, chainId: string): boolean {
  ...
  const chainNativeAddress = (CHAIN_NATIVE_TOKENS as any)[chainId];
  if (
    chainNativeAddress &&
...
function getInchChainId(chainId: string): number | undefined {
  const chainMap = {
    eth: 1,
    polygon: 137,
    optimism: 10,
  ...
  return (chainMap as any)[chainId];
}
